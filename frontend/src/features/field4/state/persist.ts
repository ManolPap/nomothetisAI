...
import type { Field4State, PersistedFileMeta, StepStatus } from './reducer'
import { initialField4State } from './reducer'

const STORAGE_KEY = 'nomothetis-field4-session-v1'
const PERSIST_EVENT = 'field4-persist-changed'

interface Field4PersistedV1 {
  v: 1
...
export function stateToPersisted(state: Field4State): Field4PersistedV1 {
  return {
    v: 1,
...
export function persistedToState(p: Field4PersistedV1): Field4State {
...
export function loadField4Persisted(): Field4State | null {
  if (typeof window === 'undefined') return null
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY)
    if (!raw) return null
    const data = JSON.parse(raw) as Field4PersistedV1
...
export function saveField4Persisted(state: Field4State): void {
  if (typeof window === 'undefined') return
  try {
...
export function clearField4Persisted(): void {
  if (typeof window === 'undefined') return
  try {
...
export interface Field4HomeMeta {
  flowCompleted: boolean
  hasSavedSession: boolean
  result: AnalyzeField4Response | null
}

export function readField4HomeMeta(): Field4HomeMeta {
...
export function field4PersistEventName(): string {
