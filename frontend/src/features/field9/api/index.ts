...
export function extractSector(file: File, signal?: AbortSignal): Promise<ExtractSectorResponse> {
  ...
  return postFormData<ExtractSectorResponse>(buildUrl('/api/field-9/extract-sector'), form, { signal, timeoutMs: 120_000 })
}
...
export function suggestIndicators(
  ...
  return postJson<SuggestIndicatorsResponse>(buildUrl('/api/field-9/suggest-indicators'), payload, { signal, timeoutMs: 120_000 })
}
...
export function fetchIndicatorData(
  ...
  return postJson<FetchDataResponse>(buildUrl('/api/field-9/fetch-data'), payload, { signal, timeoutMs: 120_000 })
}
